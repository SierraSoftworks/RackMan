...
module.exports = function(master) {
	...
	if(master.version === true && !updateVersion(master)) return master.onError(new Error('Unable to read version from .rack.json'));

	master.config = Config(path.resolve(master.project, master.version), master.project);
	...
	if(fixedVersion === true) master.monitor.watch('.rack.json', function() {
		...
		master.hooks = new Hooks(master.project);
		master.onNewConfig(Config(path.resolve(master.project, master.version), master.project));
		master.hooks.clusterChanged('.rack.json');
		master.emit('modified', '.rack.json', 'changed');
	});
