	...
	fs.stat(path.resolve(this.project, newVersion), (function(err, stat) {
		...
		this.save((function(err) {
			...
			if(this.config.resources)
				return linkSafe(
					path.resolve(this.project, newVersion, this.config.resources), 
					path.resolve(this.project, this.config.resources), 
					function(err) {
						if(err) return callback(err);
...
Admin.prototype.clean = function(version, callback) {
	...
	function cleanProject() {
		fs.readdir(this.project, (function(err, files) {
			...
			var next = (function(err) {
				...
				if(file == this.config.resources) return next();
				if(~this.config.resources.indexOf(file)) return next();
	...
	if(version) {
		if(version[0] == '#') {
			var absIndex = parseInt(version.substr(1));
			if(absIndex < 0 || this.config.versions.length <= absIndex)
				return callback(new Error(version + " does not yield a valid deployment index value"));
